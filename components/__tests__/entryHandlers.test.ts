...
describe('entryHandler functions', () => {
  ...
  describe('deleteReminderHandler', () => {
    ...
    it('alerts on API error', async () => {
      (api.deleteReminder as jest.Mock).mockRejectedValueOnce(new Error('Failed to delete reminder.'));

      await deleteReminderHandler({ reminderId: '123' });
      expect(global.alert).toHaveBeenCalledWith('Failed to delete reminder.');
    });
  });
